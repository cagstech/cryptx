...
bool hashlib_PBKDF2(const uint8_t* password, size_t plen, const uint8_t* salt, size_t salt_len, uint8_t *out, size_t keylen, size_t iter_count, uint8_t alg){

    uint8_t hash_buffer[HASH_BLOCK_OUT_MAX];
    uint8_t hash_composite[HASH_BLOCK_OUT_MAX];
    size_t hash_len;
    uint8_t c[4];
    struct cryptx_hmac_ctx hmac = {0};
    struct cryptx_hmac_ctx bck;
    size_t outlen;
    uint32_t counter = 1;
    ...
    if(!cryptx_hmac_init(&hmac, password, plen, alg)) return false;
	hash_len = hmac.digest_len;
    ...
    for(outlen = 0; outlen < keylen; outlen+=hash_len, counter++){
		...
		c[2] = (counter >> 8) & 0xff;
		c[3] = counter & 0xff;
        cryptx_hmac_update(&hmac, salt, salt_len);
        cryptx_hmac_update(&hmac, c, sizeof c);
        cryptx_hmac_final(&hmac, hash_buffer);
        memcpy(hash_composite, hash_buffer, hash_len);
        for(size_t ic=1; ic<iter_count; ic++){
            memcpy(&hmac, &bck, sizeof hmac);
            cryptx_hmac_update(&hmac, hash_buffer, hash_len);
            cryptx_hmac_final(&hmac, hash_buffer);
