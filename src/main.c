...
void rmemcpy(uint8_t *restrict dst, const uint8_t *restrict src, size_t n)
{
	size_t i;
...
// sourced from https://github.com/kokke/tiny-ECDH-c/blob/master/ecdh.c
...
gf2elem_t polynomial = { 0x000000c9, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000008 };
gf2elem_t coeff_b =	{ 0x4a3205fd, 0x512f7874, 0x1481eb10, 0xb8c953ca, 0x0a601907, 0x00000002 };
gf2elem_t base_x = { 0xe8343e36, 0xd4994637, 0xa0991168, 0x86a2d57e, 0xf0eba162, 0x00000003 };
gf2elem_t base_y = { 0x797324f1, 0xb11c5c0c, 0xa2cdd545, 0x71a0094f, 0xd51fbc6c, 0x00000000 };
scalar_t base_order = { 0xa4234c33, 0x77e70c12, 0x000292fe, 0x00000000, 0x00000000, 0x00000004 };
...
ecdh_error_t ecdh_keygen(uint8_t *pubkey, uint8_t *privkey){
	/* Get copy of "base" point 'G' */
	gf2point_copy((uint32_t*)pubkey, (uint32_t*)(pubkey + BITVEC_NBYTES), base_x, base_y);
	
	/* Abort key generation if random number is too small */
	if (bitvec_degree((uint32_t*)privkey) < (CURVE_DEGREE / 2))
	{
		return ECDH_INVALID_PRIVKEY;
	}
	...
	{
		bitvec_clr_bit((uint32_t*)privkey, i);
	}
	
	/* Multiply base-point with scalar (private-key) */
	gf2point_mul((uint32_t*)pubkey, (uint32_t*)(pubkey + BITVEC_NBYTES), (uint32_t*)privkey);
...
ecdh_error_t ecdh_compute_secret(const uint8_t *privkey, const uint8_t *rpubkey, uint8_t *secret){
	if(
	   gf2point_is_zero((uint32_t*)rpubkey, (uint32_t*)(rpubkey + BITVEC_NBYTES)) ||
	   !gf2point_on_curve((uint32_t*)rpubkey, (uint32_t*)(rpubkey + BITVEC_NBYTES)))
		return ECDH_INVALID_RPUBKEY;
	...
	gf2point_mul((uint32_t*)secret,(uint32_t*)(secret + BITVEC_NBYTES), (const uint32_t*)privkey);
	return ECDH_OK;
}
