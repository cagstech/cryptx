...
pkcs_error_t cryptx_pkcs8_import_privatekey(struct cryptx_pkcs8_privatekey *pkcs,
                                           void *data, size_t len, const char *passphrase){
  if((data == NULL) ||
     (len == 0) ||
     (pkcs == NULL)) return PKCS_INVALID_ARG;
  if(sizeof(pkcs) < len) return PKCS_INVALID_STRUCT;
  ...
  // verify that PEM banner matches expectation
  bool encrypted;
  char *test_str1 = "-----BEGIN PRIVATE KEY-----";
  char *test_str2 = "-----BEGIN ENCRYPTED PRIVATE KEY-----";
  if(strncmp(data, test_str1, strlen(test_str1))==0)
    encrypted = false;
  else if(strncmp(data, test_str2, strlen(test_str2))==0)
    encrypted = true;
  else return PKCS_INVALID_DATA;
  ...
  for(int i=0; i<=PKCS_PRIVKEY_PRIMEINFO; i++){
    err = cryptx_asn1_decode(unwrap2, unwrap2_len, i, &tag, &object_len, &object);
    if(err || (cryptx_asn1_gettag(tag) != _pkcs_validator_privatekey[i]))
      return PKCS_INVALID_DATA;
    pkcs->privatekey.info[i].data = &pkcs->raw_data[w_offset];
    pkcs->privatekey.info[i].len = object_len;
    memcpy(&pkcs->raw_data[w_offset], object, object_len);
    w_offset += object_len;
  }
  return PKCS_OK;
