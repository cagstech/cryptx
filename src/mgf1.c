...
bool hashlib_MGF1Hash(uint8_t* data, size_t datalen, uint8_t* outbuf, size_t outlen, uint8_t alg){
	...
	struct cryptx_hash_ctx ctx_data, ctx_ctr;
	if(!cryptx_hash_init(&ctx_data, alg)) return false;
	hash_len = ctx_data.digest_len;
	cryptx_hash_update(&ctx_data, data, datalen);
	for(size_t printlen=0; printlen<outlen; printlen+=hash_len, ctr++){
		...
		cryptx_hash_update(&ctx_ctr, ctr_data, 4);
		cryptx_hash_final(&ctx_ctr, hash_digest);
