...
size_t hashlib_RSAEncodeOAEP(const uint8_t* in, size_t len, uint8_t* out, size_t modulus_len, const uint8_t *auth, uint8_t alg){
	...
	// seed next 32 bytes
	csrand_fill(&out[ENCODE_SALT], hash_len);
	
	// hash the authentication string
	if(!hash_init(&ctx, alg)) return 0;
	if(auth != NULL) hash_update(&ctx, auth, strlen(auth));
	hash_final(&ctx, &out[encode_lhash]);	// nothing to actually hash
	...
	hash_mgf1(&out[ENCODE_SALT], hash_len, mgf1_digest, db_len, alg);
    ...
    hash_mgf1(&out[encode_lhash], db_len, mgf1_digest, hash_len, alg);
...
size_t hashlib_RSADecodeOAEP(const uint8_t *in, size_t len, uint8_t* out, const uint8_t *auth, uint8_t alg){
   ...
   // memcpy(salt, &in[len-RSA_SALT_SIZE-1], RSA_SALT_SIZE);
    ...
    // SHA-256 hash db
    if(!hash_mgf1(&tmp[encode_lhash], db_len, mgf1_digest, hash_len, alg)) return 0;
    ...
    // MGF1 hash the salt
    hash_mgf1(&tmp[ENCODE_SALT], hash_len, mgf1_digest, db_len, alg);
	...
	if(!digest_compare(sha256_digest, out, hash_len)) return 0;
...
rsa_error_t hashlib_RSAEncrypt(const uint8_t* msg, size_t msglen, uint8_t *ct, const uint8_t* pubkey, size_t keylen, uint8_t oaep_hash_alg){
    uint8_t spos = 0;
